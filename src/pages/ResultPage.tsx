...
export default function ResultPage() {
  ...
  const partLabel =
    partNum === 0 ? "LC 전체 문제 Test" :
    partNum === -1 ? "RC 전체 문제 Test" :
    partNum === -2 ? "LCRC 전체 문제 Test" :
    partNum === 1 ? "사진 묘사 (Photographs)" :
    partNum === 2 ? "질의 응답 (Question-Response)" :
    partNum === 3 ? "짧은 대화 (Short Conversations)" :
    partNum === 4 ? "짧은 담화 (Short Talks)" :
    partNum === 5 ? "단문 빈칸 채우기 (Incomplete Sentences)" :
    partNum === 6 ? "장문 빈칸 채우기 (Text Completion)" :
    partNum === 7 ? "독해 (Reading Comprehension)" : `Part ${part}`;
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-6">
          <h1 className="text-2xl font-bold text-gray-900">
            {isTestMode
              ? (partNum <= 0 ? partLabel : `Part ${part} Set Test`)
              : `Part ${part}: ${partLabel}`}
          </h1>
